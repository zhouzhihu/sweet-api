<template>
  <div class="ma-tree-wrapper">
    ...
    <sweet-tree :data="tree" :forceUpdate="forceUpdate" :loading="showLoading">
      <template #folder="{ item }">
        <div
            ...
            @dragover.prevent
        >
          <i :class="item.opened ? 'ma-icon-arrow-bottom' : 'ma-icon-arrow-right'" class="ma-icon" @click.stop="" @click="$set(item, 'opened', !item.opened)"/>
      ...
      <template #file="{ item }">
        <div
            ...
            :draggable="true"
            :id="'sweet-api-list-' + (item.id || item.tmp_id)"
            :style="{ 'padding-left': 17 * item.level + 'px' }"
            class="ma-tree-hover"
            ...
            @dragover.prevent
        >
          <sweet-text-icon v-model="item.method"/>
          <label>{{ item.name }}</label>
          <span>({{ item.path }})</span>
    ...
    <magic-dialog v-model="createGroupObj.visible" :title="createGroupObj.id ? '修改分组:' + createGroupObj.tmpName : '创建分组'"
      ...
      <template #content>
        <label>分组名称：</label>
        <sweet-input v-model="createGroupObj.name"/>
        <div style="height: 2px"></div>
        <label>分组前缀：</label>
        <sweet-input v-model="createGroupObj.path"/>
      </template>
      <template #buttons>
...
import SweetTextIcon from "@/components/common/sweet-text-icon";
  ...
  methods: {
    ...
    initData() {
      ...
      return new Promise((resolve) => {
        request.send(`/${contants.ESB_SERVICE_NAME}/api/list/folder`, [], {method: 'GET'}).success(data => {
          this.listGroupData = data || []
          bus.$emit('status', '接口分组加载完毕')
          request.send(`/${contants.ESB_SERVICE_NAME}/api/list/api`, [], {method: 'GET'}).success(data => {
    ...
    sync() {
      bus.$emit('status', '正在刷新接口缓存')
      return new Promise((resolve) => {
        request.send(`/${contants.ESB_SERVICE_NAME}/api/sync`, [], {method: 'GET'}).success(data => {
          bus.$emit('status', '接口缓存刷新完毕')
        })
      })
    },
    getGroup(groupId) {
    ...
    initTreeData() {
      ...
      this.listGroupData.forEach(element => {
        ...
        if ((element.path !== null)) {
          element.tmpPath = element.path && element.path.indexOf('/') === 0 ? element.path : '/' + element.path
        } else {
          element.tmpPath = "";
        }
      })
      ...
      this.listChildrenData.forEach((element, index) => {
        ...
        element._type = 'api'
        if (groupItem[element.parentId]) {
          let group = this.getGroup(element.parentId)
          element.groupName = group.name
          element.groupPath = group.path
          groupItem[element.parentId].push(element)
        } else {
          element.groupName = ''
          element.groupPath = ''
      ...
      let buildHandle = (arr, parentItem, level) => {
        arr.forEach(element => {
          ...
          if (element.folder === true) {
            ...
            element.tmpPath = (parentItem.tmpPath + '/' + ((element.path && null !== element.path) ? element.path.replace(/\s/g,"") : "")).replace(new RegExp('(/)+', 'gm'), '/')
    ...
    folderRightClickHandle(event, item) {
      ...
      this.$magicContextmenu({
        menus: [
          ...
          //     request.send(`/${contants.ESB_SERVICE_NAME}/download?groupId=${item.id}`,null,{
          //       headers: {
    ...
    fileRightClickHandle(event, item) {
      ...
      this.$magicContextmenu({
        menus: [
          ...
          //     request.send(item.lock === '1' ? `/${contants.ESB_SERVICE_NAME}/unlock` : `/${contants.ESB_SERVICE_NAME}/lock`, {id: item.id}).success(data => {
          //       if (data) {
    ...
    deleteApiInfo(item) {
      ...
      this.$magicConfirm({
        ...
        onOk: () => {
          if (item.id) {
            request.send(`/${contants.ESB_SERVICE_NAME}/api`, {ids: item.id}, {method: 'DELETE'}).success(data => {
              if (data) {
    ...
    createGroupAction(flag) {
      if (flag === true) {
        ...
        if (this.createGroupObj.id) {
          requestGroup(`/${contants.ESB_SERVICE_NAME}/api`, this.createGroupObj).success(data => {
            ...
            this.rebuildTree()
            this.$nextTick(() => {
              goToAnchor('#sweet-api-list-' + this.createGroupObj.id)
            })
          ...
          requestGroup(`/${contants.ESB_SERVICE_NAME}/api`, this.createGroupObj).success(data => {
            this.createGroupObj.id = data.id
    ...
    copyPathToClipboard(fileItem, relative) {
      let path = replaceURL((relative ? '' : contants.BASE_URL + "/" + contants.ESB_SERVICE_NAME) + '/' + fileItem.groupPath + '/' + fileItem.path)
      try {
          ...
          if (this.draggableTargetItem.folder === true) {
            ...
            if (this.draggableItem.folder === true && this.draggableItem.parentId !== this.draggableTargetItem.id && this.draggableItem.id !== this.draggableTargetItem.id) {
              ...
              if (checkChildrenFolder(this.draggableItem.children) === false) {
                ...
                request.send(`/${contants.ESB_SERVICE_NAME}/api/${this.draggableItem.id}/move`, {groupId: this.draggableTargetItem.id}).success(data => {
                  bus.$emit('report', 'group_update')
                  // 先删除移动前的分组
                  ...
                  this.changeForceUpdate()
                  this.$nextTick(() => {
                    goToAnchor('#sweet-api-list-' + this.draggableItem.id)
                  })
              ...
              if (this.draggableTargetItem.children.some(item => item.id === this.draggableItem.id) === false) {
                ...
                request.send(`/${contants.ESB_SERVICE_NAME}/api/${this.draggableItem.id}/move`, {
