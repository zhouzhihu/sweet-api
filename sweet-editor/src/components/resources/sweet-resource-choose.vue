<template>
    <sweet-tree :data="tree" :forceUpdate="forceUpdate" :style="{ height, maxHeight}" style="overflow: auto" :loading="showLoading > 0">
      <template #folder="{ item }">
        ...
        >
          <sweet-checkbox v-model="item.selected" :checked-half="item.checkedHalf" @change="e => doSelected(item,e)"/>
      ...
      <template #file="{ item }">
        ...
        >
          <sweet-checkbox v-model="item.selected" @change="e => doSelected(item,e)"/>
          <sweet-text-icon v-if="item._type === 'api'" v-model="item.method"/>
          <sweet-text-icon v-if="item._type === 'function'" value="function"/>
  ...
  methods: {
    ...
    initData() {
      ...
      this.listGroupData = [
          ...
          { id: 'datasource',_type: 'root', name: '2.数据源', parentId: 'root', path:'', selected: false, checkedHalf: false}
      ]
      request.send(`/${contants.ESB_SERVICE_NAME}/api/list/folder`, [], {method: 'GET'}).success(data => {
        data = data || []
        this.listGroupData.push(...data.map(it => {
          it.parentId = it.parentId === 0 || null === it.parentId || undefined === it.parentId ? 'api' : it.parentId;
          it.selected = false;
    ...
    initTreeData() {
      ...
      this.listGroupData.forEach(element => {
        ...
        // 缓存一个name和path给后面使用
        element.tmpName = element.name && element.name.indexOf('/') === 0 ? element.name : '/' + element.name
      ...
      this.listChildrenData.forEach((element, index) => {
        ...
        } else if (groupItem[element.groupId]) {
