      ...
      <div class="ma-layout-container" v-show="!showLoading">
        ...
        <div class="ma-content">
          <div v-for="(item, key) in todoList" :key="'todo_' + key" class="ma-table-row content-bg" @click="openItem(item)">
            <div>
              <sweet-text-icon v-if="item.type === 1" v-model="item.cache.method"/>
              <sweet-text-icon v-if="item.type === 2" value="function"/>
...
export default {
  ...
  methods: {
    getTodoList() {
      ...
      // request.send(`/${contants.ESB_SERVICE_NAME}/todo`).success(data => {
      //   this.todoList = data
