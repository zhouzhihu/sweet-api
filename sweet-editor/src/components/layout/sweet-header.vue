<template>
  <div class="ma-header">
    ...
    <span title="系统配置" @click="doSetting">
      <i class="ma-icon ma-icon-settings"></i>
    </span>
    <span v-if="user && user.userId && user.nickName" @click="logout" :title="user.nickName">
      <i class="ma-icon ma-icon-logout"></i>
    </span>
    ...
    <magic-dialog v-model="exportVisible" title="导出" align="right" :moveable="false" width="340px" height="490px"
      ...
      <template #content>
        <sweet-resource-choose ref="resourceExport" height="400px" max-height="400px"/>
      </template>
      <template #buttons>
    ...
    <magic-dialog title="远程推送" v-model="showPushDialog" align="right" :moveable="false" class="ma-remote-push-container ma-tree-wrapper"
      ...
      <template #content>
        <sweet-resource-choose ref="resourcePush" height="400px" max-height="400px"/>
        <div>
          <label>远程地址：</label>
          <sweet-input placeholder="请输入远程地址" v-model="target" width="300px"/>
        </div>
        <div>
          <label>秘钥：</label>
          <sweet-input placeholder="请输入秘钥" v-model="secretKey" width="300px" type="password" />
        </div>
      </template>
      <template #buttons>
    ...
    <sweet-search ref="search" style="flex: none"></sweet-search>
  </div>
</template>
...
import SweetResourceChoose from '@/components/resources/sweet-resource-choose.vue'
import SweetSearch from './sweet-search.vue'
  ...
  mounted() {
    ...
    bus.$on('login',() => {
      if ("" !== contants.AUTH_SERVICE_NAME) {
        ...
        request.send(`/${contants.AUTH_SERVICE_NAME}/current/user`, [], {method: 'GET'}).success(user => this.user = user)
      } else {
        bus.$emit('status', '无需登录，不需要获取用户信息')
      }
    })
  ...
  methods: {
    ...
    doExport() {
      ...
      if (selected.length > 0) {
        ...
        request.send(`/${contants.ESB_SERVICE_NAME}/api/export`, JSON.stringify(selected), {
          method: 'post',
          headers: {
          ...
          downloadFile(blob, 'sweet-api.zip')
    ...
    doPush(mode) {
      ...
      let _push = () => {
        ...
        request.send(`/${contants.ESB_SERVICE_NAME}/push`, JSON.stringify(selected), {
          method: 'post',
          headers: {
    ...
    doUpload(mode) {
      ...
      if (file) {
        ...
        let _upload = () => {
          ...
          request.send(`/${contants.ESB_SERVICE_NAME}/api/upload?mode=${mode}`, formData, {
            method: 'post',
            headers: {
    ...
    logout(){
      ...
      this.$magicConfirm({
        title: '注销登录',
        content: `是否要注销登录「${this.user.nickName}」`,
        onOk: () => {
          request.send(`/${contants.AUTH_SERVICE_NAME}/logout/token`, {token: contants.HEADER_SWEET_TOKEN_VALUE, deviceId: -1}).success(() => {
            this.user = null;
            contants.HEADER_SWEET_TOKEN_VALUE = 'unauthorization';
