    ...
    <magic-dialog title="全局搜索" :value="showDialog" align="right" width="700px" padding="0" @onClose="close()">
      <template #content>
        <div class="ma-search-container">
          <sweet-input v-model="inputText" width="100%" placeholder="请输入关键字进行搜索"></sweet-input>
  ...
  methods: {
    ...
    getResult(text) {
      ...
      if (text) {
        this.searchListFlag = setTimeout(() => {
          request.send(`/${contants.ESB_SERVICE_NAME}/api/search`,{ keyword: text }, { method: 'POST' }).success(data => {
    ...
    getDetail() {
      if (this.selectedItem.id) {
        request.send(`/${contants.ESB_SERVICE_NAME}/api/${this.selectedItem.id}/script`, [], {method: 'GET'}).success(data => {
