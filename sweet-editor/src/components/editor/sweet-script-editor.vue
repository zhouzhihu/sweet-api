    ...
    <div class="ma-wrapper">
      <ul ref="scrollbar" class="ma-tab not-select">
        <li
            ...
            @dragover.prevent
        >
          <sweet-text-icon v-if="item._type === 'api'" v-model="item.method" style="margin-top: -4px"/>
          <sweet-text-icon v-if="item._type !== 'api'" value="function" style="margin-top: -4px"/>
...
import {initializeSweetScript} from '@/scripts/editor/sweet-script.js'

export default {
  name: 'SweetScriptEditor',
  components: {
    SweetTextIcon
  },
  data() {
  ...
  methods: {
    ...
    open(item) {
      ...
      if (item.copy !== true && (info || isNew)) {
        ...
        request.send(`/${contants.ESB_SERVICE_NAME}/api/${id}/info`, [], {method: 'GET'}).success(data => {
          if (isApi) {
            ...
            let tmp = {
              children: data.requestBodyDefinition,
              dataType: "Object",
              description: "",
              error: "",
              expression: "",
            ...
            this.$set(item, 'requestBodyDefinition', tmp)
    ...
    doSaveApi() {
      ...
      saveObj.type = "api"
      saveObj.parentId = saveObj.groupId
    ...
    internalTest() {
      ...
      let requestConfig = {
        baseURL: contants.BASE_URL + "/" + contants.ESB_SERVICE_NAME,
    ...
    sendTestRequest(target, requestConfig, sessionId) {
      ...
      requestConfig.headers[contants.HEADER_REQUEST_SESSION] = contants.HEADER_REQUEST_SESSION_VALUE
      requestConfig.headers[contants.HEADER_SWEET_TOKEN] = contants.HEADER_SWEET_TOKEN_VALUE
