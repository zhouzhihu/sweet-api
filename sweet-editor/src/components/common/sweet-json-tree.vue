   ...
   <ul class="ma-tree">
    <li v-for="(item, index) in jsonData" :key="index" @click="handleItemClick(item, index)">
      ...
      <div class="item-inline"  :class="item.selected ? 'tree-item item-selected' : 'tree-item'">
        <sweet-json-tree-format :item="item" :index="index" :dataLength="jsonData.length" :indentLevel="indentLevel"/>
      ...
      <template  v-if="item.dataType === 'Object' || item.dataType === 'Array'" >
        <sweet-json-tree :jsonData="item.children" :indentLevel="indentLevel | createLevel(jsonData, item, index)" :forceUpdate="forceUpdate" v-on="$listeners"/>
      </template>
    </li>
  </ul>
</template>
...
<script>
  import SweetJsonTreeFormat from './sweet-json-tree-format'
